...
class RequestedServices extends Controller {
        ...
        if ($request['currentStatus'] != 1) {
            ...
            if (Input::get("approveIt") !== null) {
                if (Input::get("supplier_id") != null) {
                    ...
                    $query = "SELECT users.id, users.first_name, users.last_name,  users.email, users.mobile_number, users.approval_email from  users , suplier_buyerrel "
                    ...
                    $query = "SELECT id, first_name, last_name, service_kilometers, email , mobile_number, approval_email 
                ...
                foreach ($suppliers as $supplier) {
                    ...
                    if ($subscriptions && $subscriptions->quote_notification) {
						...
						$message = "A new request has been posted on Qondo. Send quote here ".url('project-details/' . $requestDetails['id']);
						...
						if(!empty($mobileNumber)){
							app('App\Http\Controllers\TwilioVerifyController')->sendTwilioSMS($mobileNumber, $message);
							//echo $phoneNumber.'<br>';exit;
						}
                    }
                }
            ...
            if ($subscriptions && $subscriptions->request_notification) {
				//DB::enableQueryLog();
                $buyerDetails = User::select('first_name', 'last_name', 'email', 'approval_email')->where('id', $requestDetails['buyer_id'])->first();//dd(DB::getQueryLog());//->toArray();
    ...
    public function updateRequest($id, Request $request) {
		...
		$service_data = DB::table('request_service')
                ->where('id', $id)
                ->first();
		//print_r($service_data);
		echo $service_data->buyer_id;
		$buyer_data = DB::table('users')
                ->where('id', $service_data->buyer_id)
                ->first();
				print_r($buyer_data);
