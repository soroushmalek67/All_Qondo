...
//        If supplier approved by admin, temporary account will be assigned to supplier
    ...
    public function Delete() {
		...
		DB::table('request_service')->where('supplier_id', $userid)->orwhere('buyer_id', $userid)->delete();
		DB::table('quotes')->where('supplier_id', $userid)->delete();
		DB::table('quotes_invitation')->where('supplier_id', $userid)->delete();
		DB::table('invoices')->where('supplier_id', $userid)->orwhere('buyer_id', $userid)->delete();
		DB::table('refund')->where('user_id', $userid)->delete();
		DB::table('messages')->where('sender_id', $userid)->delete();
		DB::table('products_services')->where('users_id', $userid)->delete();
		DB::table('promotion_coupons')->where('user_id', $userid)->delete();
		DB::table('reviews_testimonials')->where('user_id', $userid)->orwhere('reviewer_id', $userid)->delete();
		DB::table('transactions')->where('userid', $userid)->delete();
		DB::table('emails_subscription')->where('userid', $userid)->delete();
		
		DB::table('users')->where('added_by', $userid)->update(['added_by' => 0]);
		
		AdminController::LogAdminActivity('Users', 'delete');
