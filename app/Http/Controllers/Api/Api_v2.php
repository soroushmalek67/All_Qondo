...
class Api_v2 extends Controller
{
	...
	*/
	...
	/*
	---------------------------------------------
		FUNCTION:	Categories List with Parent ID - ANDROID
		
		METHOD:		GET
		
		URL: 		url('api/get-categories-list')
	...
	*/
	public function GetCategoriesList()
	{
		$cats = DB::table('category_description')->select("category_id", "name", 'parent_id', "description", "category.slug", "category.category_icon")
				...
				//->selectRaw(DB::raw('category_description.name AS category_name')->where('category_description.category_id', '=', 'category. parent_id'))
		...
		foreach ($cats as $cat)
		{
			if ($cat->parent_id != 0) 
			{
				$response['suggestions'][] = array("data" => $cat->category_id, "value" => $cat->name, "slug" => $cat->slug, "optionType" => 'category', 'type' => 'service', 'pid' => $cat->parent_id, 'icon' => $cat->category_icon);
			}
			else
			{
				$response['suggestions'][] = array("data" => $cat->category_id, "value" => $cat->name, "slug" => $cat->slug, "optionType" => 'category', 'type' => 'category', 'pid' => $cat->parent_id, 'icon' => $cat->category_icon);
			}
		}
	...
	*/
	public function ViewProfile()
	{
		...
		if(File::exists(public_path('img/users/'.$use_id.'.jpg')))
		{
			$user_image = $use_id.'.jpg';
		}
		
		$data['user_info']['user_image'] = url('img/users/'.$user_image);
