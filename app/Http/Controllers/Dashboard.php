    ...
    public function index(Request $request) {
      ...
      $data['userDetails'] = User::find(Auth::id());
        ...
        if ($data['userType'] == 1) {
			...
			$data['requests'] = User::find($this->userid)->requestedServices()
                        ->leftJoin(DB::raw('(SELECT request_id, price, status, SUM(view_status = 0) as quotesNew, COUNT(id) as quotesTotal FROM quotes group by request_id) as q'), 'q.request_id', '=', 'request_service.id')
                        ->select('request_service.*', 'q.quotesNew', 'q.quotesTotal', 'q.price', 'q.status as quoteStatus')->orderBy('id', 'desc')->get()->toArray();
...
//                                    ."AND r.buyer_id = ".$this->userid." AND q.status = 1 group by r.id ORDER BY r.id DESC");
			...
			$quotesAlert = DB::table('quotes')->leftJoin('request_service', 'quotes.request_id', '=', 'request_service.id')->where('quotes.alert_status', 0)->where('request_service.buyer_id', $this->userid)->count();
			if($quotesAlert > 0)
			{
				if($quotesAlert > 1){ $qMessage = 'You recieved new quotes.'; } else { $qMessage = 'You recieved a new quote.'; }
				$request->session()->flash('message', $qMessage.' Please check alert below');
				DB::table('quotes')->leftJoin('request_service', 'quotes.request_id', '=', 'request_service.id')->where('request_service.buyer_id', $this->userid)->update(['quotes.alert_status' => 1]);
			}
		} else {
            $quotesInvitations = DB::table('quotes_invitation')->where('notification_status', 0)->where('supplier_id', $this->userid)->count();
			
			if($quotesInvitations > 0)
			{
				$request->session()->flash('message', 'Congratulation! You recieved new inviation(s) to quote.');
				DB::table('quotes_invitation')->where('notification_status', 0)->where('supplier_id', $this->userid)->update(['notification_status' => 1]);
			}
			$quoteAccepted = DB::table('quotes')->where('accept_status', 1)->where('supplier_id', $this->userid)->count();
			
			if($quoteAccepted > 0)
			{
				$request->session()->flash('message', 'Congratulation! Your quote has been accepted.');
				DB::table('quotes')->where('accept_status', 1)->where('supplier_id', $this->userid)->update(['accept_status' => 2]);
			}
...
//        });
    ...
    public function projectDetails($id) {
        ...
        if ($data['userType'] == 1) {
			...
			DB::table('quotes')->where('request_id', $id)->update(['view_status' => 1]);
