              ...
              <table id="example2" class="table table-bordered table-hover">
                <thead>
                    <tr>
                        ...
                        <th width="10">
                        	Estimated Budget
                        	<?php $budgettypes = array('small' => 'Small ($50 - $249)', 'medium' => 'Medium ($250 - $999)', 'large' => 'Large ($1000+)'); ?>
                        </th>
                        ...
                        {{--*/ $loopCounter++ /*--}}
                            <tr @if ($requestedService['when_need_it'] == 2) class="bg-success" @endif>
                                ...
                                <td style="color: {{($requestedService['status'] == 0) ? "#00c0ef" : (($requestedService['status'] == 1) ? "green" : "red")}}">
                                 ...
                                 <td> 
                                 	 @if ($requestedService['when_need_it'] == 2)
                                     	With in 48 hours
                                     @elseif ($requestedService['how_to_proceed'] == 1) 
                                     	{{date('F d, Y', strtotime($requestedService['when_need_it_date']))}}
                                    @else
                                    	@if ($requestedService['how_to_proceed'] == 2) 
                                    		Quotes Only
                                    	@endif
                                    @endif
