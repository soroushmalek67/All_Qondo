...
//        }
    ...
    public function userType()
    {
        ...
        $id = $this->session->get($this->getName()."_userType", $this->getRecallerId());
        if (is_null($id) && $this->user()) {
            $id = $this->user()->getAuthIdentifier();
        }
        return $id;
...
//         print_r($credentials); exit();
        ...
        array_forget($credentials, 'userType');
        $this->fireAttemptEvent($credentials, $remember, $login);
        $this->lastAttempted = $user = $this->provider->retrieveByCredentials($credentials);
        if ($this->hasValidCredentials($user, $credentials)) {
            if ($login) {
                $this->login($user, $remember, $userType);
            }
            return true;
        }
        return false;
    ...
    public function login(UserContract $user, $remember = false, $usertype = 1)
    {
        
//        printr($usertype); exit;
        $this->updateSession($user->getAuthIdentifier(), $usertype);
        if ($remember) {
            $this->createRememberTokenIfDoesntExist($user);
            $this->queueRecallerCookie($user, $usertype);
        }
        $this->fireLoginEvent($user, $remember);
        $this->setUser($user);
    }
    public function slogin(UserContract $user, $remember = false, $usertype)
    {
...
//        printr($usertype); exit;
    ...
    protected function updateSession($id, $usertype)
    {
        $this->session->set($this->getName(), $id);
        $this->session->set($this->getName()."_userType", $usertype);
    ...
    public function sloginUsingId($id, $remember = false, $usertype)
    {
        $this->session->set($this->getName(), $id);
        $this->slogin($user = $this->provider->retrieveById($id), $remember, $usertype);
        return $user;
    }
    ...
    protected function queueRecallerCookie(UserContract $user, $usertype)
    {
        $value = $user->getAuthIdentifier() . '|' . $user->getRememberToken()."|".$usertype;
...
trait AuthenticatesAndRegistersUsers
{
    ...
    public function getRegister() {
        $data['metas'] = get_page_meta_array('Sign Up');
...
//        print_r($request->all());exit();
        ...
        $userdetail= DB::table('users')
                    ->orwhere('business_name',$request->get('business_name'))
...
//                print_r($userdetail->status);exit();
                return redirect('supplier-profile/'.$userdetail->business_name.'/'.$userdetail->id.'')->with('unregisterd','1');
            }
            else{
                    $admin_email=getSettings('notification_email');
                    
                     $duplicate_companyN = DB::table('notification')
                                        ->where('notificationName','duplicate_company')
                                        ->first();    
            $buyer  = strtr($duplicate_companyN->content, ["@business" => $userdetail->business_name , "@UserEmail" =>$userdetail->email ,
                                   ]);
            
            $data = ['emailbody' => $buyer];
                        Mail::send('emails.duplicateUser', $data, function($message) use ($admin_email){
                            $message->to($admin_email)
                                ->subject('Duplicate Company');
                            $message->from('info@firmogram.com', 'Firmogram');
                        });
                
                return redirect('auth/login')->with('message', '1');
            }
        }else{
             $this->registrar->create($request->all());
              return redirect('auth/login')->with('message', 'A verification message was sent to your email address. Please verify your email and then continue.');
        }
        
       
//        $this->auth->login($this->registrar->create($request->all()), false, $request->get("iAmA"));
       
    }
    public function getLogin() {
        $data['metas'] = get_page_meta_array('Login');
        return view('auth.login', $data);
    }
    public function postLogin(Request $request)
    {
//        print_r($request->all());exit();
        $this->validate($request, array('email' => 'required|email', 'password' => 'required', 'userType' => 'required'));
        ...
        if (User::where('email', '=', $email)->exists()) {
            ...
            $userActivity = UserActivityModel::create(['email' => $request->get('email'), 'ip' => $ip]);
            
            $time30MinsBefore = date("Y-m-d H:i:s", strtotime("-30 minutes"));
            $triesWithin30Mins = DB::table(DB::raw("(SELECT ua.*, @r:= CASE WHEN @g = ua.status THEN @r ELSE @r + 1 END consecutive, @g:= ua.status g
                                                        FROM users_activity ua CROSS JOIN (SELECT @g:=0, @r:=0) t1 WHERE created_at > '$time30MinsBefore' 
                                                        ORDER BY ua.id) AS lt"))
                                        ->select('lt.email', 'lt.ip', 'lt.consecutive', 'lt.status', DB::raw('COUNT(1) AS consecutive_count'))
                                        ->groupBy('lt.consecutive', 'lt.status')->where('lt.ip', '=', $ip)->where('lt.status', '=', 0)
//                                        ->having('consecutive_count', '>=', 5)'lt.email', 
                                        ->orderBy('lt.id', 'DESC')->first();
            
            if ($triesWithin30Mins && $triesWithin30Mins->consecutive_count > 4) {
                return redirect($this->loginPath())
                            ->withInput($request->only('email', 'remember'))
                            ->withErrors([
                                    'Bloked' => "Your account is bloked temporarily.",
                            ]);
            }
        }
        ...
        if ($this->auth->attempt($credentials, $request->has('remember'))) {
            ...
            if ($this->auth->user()->user_type != 3) {
                if ($this->auth->user()->user_type != $request->get('userType')) {
                    $this->auth->logout();
                    return redirect($this->loginPath())
                                        ->withInput($request->only('email', 'remember'))
                                        ->withErrors([
            ...
            if ($this->auth->user()->status == 0 || $this->auth->user()->status == 6) {
                $this->auth->logout();
                return redirect($this->loginPath())
                                        ->withInput($request->only('email', 'remember'))
                                        ->withErrors([
                                                'email' => "please verify email befor login.",
                                        ]);
            }
            
            if ($this->redirectPath() == "/request-service/save") {
    ...
    public function getConfirm($token) {
        ...
        $user->confirmEmail();
        $userType = ($user->user_type == 3) ? 2 : $user->user_type;
        $this->auth->login($user, false, $userType);
...
trait ResetsPasswords
{
    ...
    public function getEmail() {
        $data['metas'] = get_page_meta_array('Reset Password');
